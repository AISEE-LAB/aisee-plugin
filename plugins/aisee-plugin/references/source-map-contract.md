...
JSON 字段名保持向后兼容：`Affected Paths Index` 解析后仍输出为 `implementation_paths`，`Expected Evidence Index` 解析后仍输出为 `verification_evidence`，`Contract Ownership / Sync` 解析后输出为 `contract_sync`。
...
- 旧字段名 `Contract Source`、`Frontend Consumer`、`Backend Provider`、`External Repo / Package / Artifact Path`、`Version / Commit / Tag` 可被兼容解析。
- 本表只记录同步路由和权威来源，不写接口字段、错误码或业务规则。

### Affected Paths Index

中文标题可用 `影响路径索引`、`候选影响路径`、`实现路径`、`代码路径` 或 legacy `Implementation Paths`。
...
中文标题可用 `预期证据索引`、`验证证据` 或 legacy `Verification Evidence`。
