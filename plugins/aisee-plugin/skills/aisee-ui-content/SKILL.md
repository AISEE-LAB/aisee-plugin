...
正式写法只使用文档内编号，例如 `PAGE-001`、`FLOW-001`、`STATE-001`。跨文档来源由后续 `source-map.md` 记录；不要使用旧 full ID、ID lifecycle 或 lookup/trace 流程。

如果当前轮次还不能确定最终编号，可以使用 `PAGE-NEW-001`、`FLOW-NEW-001`、`STATE-NEW-001`，并显式标注 `[NUMBERING-FINALIZATION-REQUIRED]`。不要把占位符当正式稳定编号。
...
- 正式 PAGE / FLOW / STATE 只使用文档内编号；无法确定最终编号时使用 `TYPE-NEW-001` 并标注 `[NUMBERING-FINALIZATION-REQUIRED]`。
