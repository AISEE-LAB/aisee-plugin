...
- 只有 schema 声明 `source_map_routing` 时才要求 `source-map.md`。
...
4. 项目记忆：先用 `aisee memory inspect --json` 发现状态，再用 `aisee memory search --query "<task>" --json` 受控检索。
5. `aisee/docs/reflect/`：会话复盘和 memory 候选区；只有用户明确要求提升时，才通过 `aisee memory add` 写入 `aisee/memory/`。
...
先用 `aisee memory inspect --json` 发现状态，再用 `aisee memory search --query "<task>" --json` 只加载本任务相关条目。`aisee/memory/` 是长期项目记忆的 canonical 位置；`aisee/docs/reflect/` 是会话复盘和待确认候选区，不能替代 `aisee/memory/`。
