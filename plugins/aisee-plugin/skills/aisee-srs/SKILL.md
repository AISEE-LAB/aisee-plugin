...
正式写法只使用文档内 local ID，例如 `FR-001`、`NFR-001`、`RULE-001`、`FLOW-001`、`STATE-001`。跨文档引用交给后续 `source-map.md`、`aisee get`、`aisee trace` 或 alias 形式处理，例如 `docs/requirements/auth-srs.md#FR-001`、`srs:auth-login#FR-001`。不要再要求 `scope:TYPE-001`、`aisee id reserve/activate/check` 或 `<!-- aisee:id ... -->`。

如果当前轮次还不能确定最终编号，可以使用 `FR-NEW-001` 一类 local placeholder，并显式标注 `[ID-FINALIZATION-REQUIRED]`。SRS 不负责 `PAGE / API / DATA / TASK / TEST`；`PAGE` 以及 UI 细化由 `aisee:ui-content` 承接；`API / DATA / TASK / TEST` 由 change artifacts 承接。
...
- 正式 `FR / NFR / RULE / FLOW / STATE` 只使用文档内 local ID；工具不可用时使用 `TYPE-NEW-001` 并标注 `[ID-FINALIZATION-REQUIRED]`。
