...
- 不要把 SRS 写成 UI 设计文档。页面字段、列表列、操作和空状态只作为功能合约；视觉布局和详细页面构成留给后续 change 中的页面/交互约束或按需补充文档。
...
- 标准模式 SRS 不应承载超过 10 条 FR；如果总范围超过 10 条 FR、有效业务能力模块不少于 3 个，或复杂场景扩展块超过 2 个，标记为 Epic 并进入 Epic 模式。
...
- 正式 `FR / NFR / RULE / FLOW / STATE` 只使用文档内编号；无法确定最终编号时使用 `TYPE-NEW-001` 并标注 `[NUMBERING-FINALIZATION-REQUIRED]`。
