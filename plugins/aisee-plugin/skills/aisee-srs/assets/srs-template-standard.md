...
| NFR ID | 指标 | 要求 | 备注 |
|------|------|------|
| NFR-001 | 响应时间 | {e.g., P99 < 500ms} | |
| NFR-002 | 并发用户数 | {e.g., 1000} | |
| NFR-003 | 列表查询 | {e.g., 10万条数据下 < 1s} | |
...
- 后续 change 重点：{常规 spec / 页面与交互约束 / service contract / cli contract / job contract / 设备约束说明}
- 实施前仍需确认：{Q-xxx / 无}
```
