# aisee:srs — Epic 主文档模板

仅在 Epic 模式下使用本模板。主文档不展开 Section 3 的 FR 详情，只作为全局概览和索引。

```markdown
---
title: "需求规格说明书：{系统/功能名称}"
doc_type: "srs"
status: "draft"
date: "{date}"
scope: "{scope}"
owner: "{从 AGENTS.md 或项目上下文提取，或填\"待填写\"}"
source_refs:
  - "{repo-relative source path or source ref}"
change_refs: []
---

# 需求规格说明书（主文档）

**文档编号**：SRS-{YYYY-MM-DD}-{slug}-main
**版本**：v1.0

> 正式写法只使用文档内编号，例如 `FR-001`、`RULE-001`。跨文档引用交给 `doc-ref#编号`；无法确定最终编号时使用 `TYPE-NEW-001` 临时占位符，并标注 `[NUMBERING-FINALIZATION-REQUIRED]`。

---

## 模块文档索引

| 模块名称 | 文档路径 | FR 范围 | 优先级 |
|----------|---------|---------|--------|
| {模块 A} | [`./01-{module-a}.md`](./01-{module-a}.md) | FR-001 ~ FR-00X | P0 |
| {模块 B} | [`./02-{module-b}.md`](./02-{module-b}.md) | FR-00X ~ FR-00Y | P1 |

### 模块划分依据

| 候选模块 / 输入章节 | 处理结果 | 原因 |
|---------------------|----------|------|
| {candidate} | 保留 / 合并 / 拒绝 | {能力边界 / 只是背景材料 / 只是技术层 / 只是页面类型 / 与其他模块同属一个流程} |

---

## 1. 引言

### 1.1 目的
本文档描述 {系统/功能名称} 的需求，供开发团队、产品负责人及相关干系人使用。
各能力模块的详细 FR 见上方"模块文档索引"中的对应文档。

### 1.2 范围
{两到三句话说明系统边界：它做什么、不做什么、与哪些系统交互}

### 1.3 需求域
| 项 | 内容 |
|----|------|
| 需求域 | app / web / mini-program / desktop / backend-service / cli-tool / job-async / hybrid-software |
| 交付形态 | {Web / App / 小程序 / 桌面 / 后端服务 / CLI / Job / 多端 / 设备协作软件} |
| 域识别依据 | {用户说明 / 项目上下文 / 假设} |
| 后续关注点 | change-plan / 页面与交互约束 / service contract / cli contract / job contract / 设备约束说明 |

### 1.4 术语与缩写
| 术语 | 定义 |
|------|------|
| {term} | {definition} |

### 1.5 参考文档
- {已引用文档，或填"暂无"}

---

## 2. 整体描述

### 2.0 编号状态

| 检查项 | 状态 | 证据 / 命令 | 备注 |
|---|---|---|---|
| 编号已稳定 | yes / no | 文档内编号检查 | |
| 跨文档来源已记录 | yes / no / N/A | 模块索引 / `source_refs` / `source-map.md` | |
| 存在临时编号 | yes / no | `[NUMBERING-FINALIZATION-REQUIRED]` | |

### 2.1 产品概述
{系统定位、解决的核心问题、与现有系统的关系}

### 2.2 目标用户
| 用户角色 | 描述 | 主要诉求 |
|----------|------|----------|
| {role} | {desc} | {need} |

### 2.3 业务目标与非目标
| 类型 | 内容 |
|------|------|
| 业务目标 | {本期要解决的问题、要达成的结果} |
| 成功判定 | {上线后怎样算达到目标} |
| 明确非目标 | {本期明确不做什么} |

### 2.4 业务对象与状态概览
| 业务对象 | 说明 | 关系 / 归属 | 关键状态 | 备注 |
|----------|------|-------------|----------|------|
| {object} | {description} | {relationship or owner} | {states or 无} | {note} |

### 2.5 交付形态与使用场景

> 只记录业务上可见或可调用的交付形态，不写技术实现方案。

| 形态 | 是否涉及 | 业务用途 | 后续关注点 |
|------|----------|----------|--------------|
| Web / 管理后台 / H5 / 小程序 / App / 桌面 GUI | 是/否 | {用户在哪里完成任务} | {关键页面入口、状态反馈、权限可见性、关键字段或 N/A} |
| 后端服务 / API 能力 | 是/否 | {对内/对外提供什么业务能力} | 需要 / 不需要 service contract |
| CLI / 工具 | 是/否 | {谁在什么场景调用} | 需要 / 不需要 cli contract |
| 异步任务 / 定时任务 / 导入导出 | 是/否 | {业务处理目的} | 需要 / 不需要 job contract |
| 通知 / 消息 / 外部系统协作 | 是/否 | {触发和接收场景} | 需要 / 不需要 service/job contract |
| 设备协作 / 上位机 / IoT 状态 | 是/否 | {软件侧需要感知或展示的设备能力、状态、告警、输入输出业务含义} | {后续 change 需细化的软件侧约束；纯硬件 / 固件细节进入专用流程} |

### 2.6 现有基线影响

> 仅在 baseline-aware 模式下填写。只记录业务行为层面的影响，不写接口、数据库、代码实现或迁移方案。

| 类型 | 现有行为 / 规范 | 本需求影响 | 处理方式 | 来源 |
|---|---|---|---|---|
| 新增 / 修改 / 移除 / 兼容 | {existing behavior or 无} | {impact} | {新增 FR / 修改既有行为 / 移除待确认 / 保持兼容} | {openspec/specs/... or docs/... or 用户说明} |

---

## 3. 功能需求

> 本文档不包含 FR 详情，各模块 FR 见上方"模块文档索引"中的对应文档。

---

## 4. 非功能需求

### 4.1 性能
| 指标 | 要求 | 备注 |
|------|------|------|
| 响应时间 | {e.g., P99 < 500ms} | |
| 并发用户数 | {e.g., 1000} | |
| 列表查询 | {e.g., 10万条数据下 < 1s} | |

### 4.2 安全
- {认证/授权要求}
- {数据敏感性要求，如：手机号脱敏展示}

### 4.3 可靠性
- {可用性目标，e.g., 99.9%}
- {数据持久性要求}

### 4.4 可维护性
- {日志、监控、告警要求}

### 4.5 其他
- {国际化、无障碍访问、合规要求等}

---

## 5. 约束与假设

### 5.1 已确认约束
- {constraint 1}

### 5.2 假设（完整列表）
| 编号 | 假设内容 | 影响模块 | 影响需求 | 确认状态 |
|------|----------|----------|----------|----------|
| A-001 | {assumption} | {模块名} | {FR-xxx} | 待确认 |

---

## 6. 待确认事项（Open Questions）

| 编号 | 问题 | 影响范围 | 负责人 | 截止日期 |
|------|------|----------|--------|----------|
| Q-001 | {question} | {FR-xxx} | 待指定 | 待定 |

---

## 7. 下游建议（可选）

- 是否建议直接进入 `aisee:change-plan`：是 / 否；原因：{范围已稳定 / 仍需先确认关键问题}
- 后续 change 重点：{模块 A 关注页面与交互约束；模块 B 关注 service contract；或写常规 spec / cli contract / job contract / 设备约束说明}
- 进入后续规划前仍需确认：{Q-xxx / 无}
```
