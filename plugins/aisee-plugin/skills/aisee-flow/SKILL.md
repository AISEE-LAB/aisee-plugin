...
默认推荐路径优先指向 core workflow。`design-*`、`svg-assets`、`image-object`、`spec-migrate`、`reflect`、`knowledge-curate` 和 `hw:*` 只在确有条件命中时作为按需分支出现，不得被平铺成每次都要走的固定步骤。
...
- 如果需要 Aisee reviewer lens，只推荐 `aisee-change-architect`、`aisee-spec-reviewer`、`aisee-implementation-reviewer` 这三个只读一致性审查 lens。
...
`aisee:flow` 只输出 lens 触发建议，不自动启动 reviewer runtime。
