...
- 只消费 `--for ce-work` 的 lean projection；不要把 full context pack、artifact 正文或全量治理诊断原样转发给 `ce-work`。
...
- 指出 blocker / risk / gap，但默认只做 advisory projection；除非 context pack 本身不可用，不替用户裁决是否继续实现。
...
- `apply_tracks` / 回写位置。
- 推荐下一步：`ce-work` 或 `ce-plan`，必要时补充 artifact 修订 follow-up。
...
只有当前 change 不存在、context pack 命令不可用或读取入口本身失效时，才输出 `[IMPLEMENTATION-BRIDGE-BLOCKED]`。其它 schema / artifact / traceability 问题默认保留在 JSON `gaps` 与 Brief 风险段，交由用户或执行者决定是否先修补。

CHECKPOINT: 推荐进入 `ce-work` 前，必须确认 context pack 可用，且执行所需范围至少能从当前 change / context pack 投影出来。scope、apply tracks、实现路径或 schema 问题应进入 advisory JSON 与 Brief 风险段；只有读取入口本身失效时才停止并输出 `[IMPLEMENTATION-BRIDGE-BLOCKED]`。

## 推荐读取顺序

优先使用 Aisee CLI 基于当前 change 生成最小上下文包。核心入口：
...
- `context pack` 返回 `SCHEMA_METADATA_MISSING` / `SCHEMA_MISMATCH` / `SCHEMA_NOT_INSTALLED` / `SCHEMA_NOT_FOUND`：默认保留为 advisory gap，并在结果中明确风险；不要自动把实现交接打回上游。
- 如果 `context pack` 读取的是官方或轻量 schema，且缺少 Aisee 专属增强字段，不把这类缺口升级成 blocker；继续根据 schema 已声明的 artifacts、`apply.tracks`、`source-map.md` 是否存在来判断执行入口。
