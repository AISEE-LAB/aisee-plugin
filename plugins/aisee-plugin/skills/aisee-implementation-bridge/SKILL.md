...
- schema metadata 已声明且项目内已安装该 schema；`SCHEMA_METADATA_MISSING`、`SCHEMA_MISMATCH`、`SCHEMA_NOT_INSTALLED`、`SCHEMA_NOT_FOUND` 都是实现 blocker。
...
如果门禁不满足，输出 `[IMPLEMENTATION-BRIDGE-BLOCKED]`，列出 context pack blocker、缺失 artifact 和范围问题，并建议先修补当前 change，再决定是否进入 `ce-work`。

CHECKPOINT: 推荐进入 `ce-work` 前，必须确认 context pack 可用、当前 change scope 清楚、apply tracks 存在或当前 schema 明确不需要、实现路径来自 context pack / schema artifacts。任一条件不满足时停止并输出 `[IMPLEMENTATION-BRIDGE-BLOCKED]`；不得生成可执行 Brief，不得建议 `ce-work`。
...
- `context pack` 返回 `SCHEMA_METADATA_MISSING` / `SCHEMA_MISMATCH` / `SCHEMA_NOT_INSTALLED` / `SCHEMA_NOT_FOUND`：停止，不生成可执行 Brief。
