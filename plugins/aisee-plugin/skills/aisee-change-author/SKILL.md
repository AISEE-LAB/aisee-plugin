...
- 读取当前 schema、change 目录和已存在 artifacts 做 schema-aware 检查；不要求用户手工执行常规命令。
- 按 schema `artifacts[].requires` 生成或补齐 artifacts。
- 当前 schema 生成 `source-map.md` 时，用它串联上游来源、产出编号、artifact 适用性、路径和验证证据。
- 当前 schema 不生成 `source-map.md` 时，不伪造 source-map；用该 schema 的主 artifact 承载缺口、方案、调研、文档或运维信息。
- 需要新增标识时，只写当前 change 内的文档内编号；跨文档来源统一回写到 `source-map.md`。
...
CHECKPOINT: 写入或修改任何 artifact 前，必须输出当前 change、schema、artifact order、待写文件、编号处理动作、已有 artifact 合并策略和 blocker/gap 摘要，等待用户确认。未确认时只能输出 author 草稿、patch 预览或缺口报告，不直接写入 change 目录。
...
7. 写入后再次检查当前 change 与 schema 是否一致。

详细 artifact 编写边界、app schema v2 顺序、N/A 规则、编号规则和无 source-map schema 落点见：
...
```text
schema preflight -> blocker / schema / numbering check
proposal.md -> change scope from confirmed change-plan
source-map.md -> only when schema generates it; upstream sources + produced numbers + artifact applicability
