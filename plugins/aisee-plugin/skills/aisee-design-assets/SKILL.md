...
description: 通用视觉参考图、设计证据与生成型素材工作流。用于用户要求基于描述生成 App、微信小程序、H5、PC Web、桌面端或多端参考图；从参考图/截图提取 StyleSpec 草稿或视觉线索；基于既有 design-spec / StyleSpec 生成新的参考图；规划参考图中的素材需求；生成背景、插画、装饰层、布局视觉层和生成型透明素材；整理项目本地图片索引、视觉来源索引和素材清单时触发。不要用于 SRS、UI 内容规格、长期 UI 设计规范事实源、前端实现、Figma 写入、基于已有图片的对象分离、素材提取、去背景、mask 生产、背景修补或图层包生成；这些交给 aisee:image-object。长期设计规范应作为独立 design spec / delta planning doc 交给 aisee:design-spec。
---
...
- 图片生成、图生图视觉变体、透明生成型素材、CLI fallback：读 `references/image-generation.md`
...
5. 有参考图/规范且要素材：生成素材清单；生成型背景、插画、装饰层由本 skill 生成，基于已有图片提取的对象素材转交 `aisee:image-object`。
...
- 内置 `image_gen` 可用时，普通参考图、视觉变体、已有 UI 效果图保结构重绘和生成型透明素材优先走内置工具。
- 内置工具不可用，或用户明确要求脚本、Images API、模型、批量、原生透明或固定输出格式时，才走 CLI fallback。
- 项目绑定图片最终必须复制或移动到项目本地。
- 真实 API 调用必须使用用户显式配置的 `api_key` 和 `base_url`；运行时数据写入目标项目目录。
- 通用图标、导航图标、操作图标和状态图标优先使用成熟图标库；图片模型只生成占位或视觉氛围。
- 图生图视觉变体默认是受控修改，必须保护品牌元素、布局结构、视觉风格和不应变动区域。需要 mask 的局部编辑由 `aisee:image-object` 准备 workspace 和 handoff。
