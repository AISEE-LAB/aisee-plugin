...
> 当前状态：early alpha / pre-release。`0.1.0` 已通过 TestPyPI、venv 安装和隔离 pipx 安装验证；正式 PyPI 发布和 1.0 兼容冻结仍在完善中。
...
- 发布 `aisee-plugin` 到正式 PyPI。
- 在干净环境中验证从正式 PyPI 执行 `pipx install aisee-plugin`。
- 将安装说明从 pre-release 指引更新为公开安装指引。

### 1.0

- 冻结 CLI JSON、schema packs、context packs、plugin export 和 skill contracts 的兼容边界。
- 明确破坏性变更的弃用、迁移和版本升级规则。
