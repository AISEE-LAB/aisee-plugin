...
| `.agents/plugins/marketplace.json` | Codex marketplace listing，指向仓库 plugin root | 仓库已提供 |

`.codex-plugin/plugin.json` 属于插件本体，声明：
...
`.agents/plugins/marketplace.json` 属于 marketplace entry，source 指向仓库根目录：
...
- `.agents/plugins/marketplace.json` 指向仓库 plugin root；
