...
> 实验性功能：当前适合本地试用和工作流 dogfood，不建议作为公开稳定 contract 依赖。远程仓库同步和 promote-batch 已可用；默认模板位于 marketplace-installed plugin 的 `skills/aisee-knowledge-curate/assets/team-knowledge/`，也可以使用外部知识库；不再由 PyPI CLI scaffold。PR 自动化和 MCP 服务仍未稳定。
...
- 通过 marketplace-installed plugin 的 `skills/aisee-knowledge-curate/assets/team-knowledge/` 或外部 Git 仓库获取 team knowledge 模板；
...
如需默认模板，先通过 Codex marketplace 安装 Aisee plugin，再从插件内容 `skills/aisee-knowledge-curate/assets/team-knowledge/` 复制到独立 team knowledge 仓库；CLI 不再从 wheel 复制 scaffold。
