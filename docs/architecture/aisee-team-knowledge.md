...
推荐独立仓库。默认模板位于 GitHub-backed Codex marketplace 中 Aisee plugin 的 `skills/aisee-knowledge-curate/assets/team-knowledge/`；PyPI CLI 不再从 wheel 复制本地 scaffold。初始化时应先安装 marketplace plugin 并复制该模板目录，或从团队维护的独立知识仓库获取模板。
...
`aisee knowledge check --team-path <path> --json` 校验 card / pack 的最小机器协议，包括必填字段、status、glob 安全、重复 ID 和 deprecated replacement。
