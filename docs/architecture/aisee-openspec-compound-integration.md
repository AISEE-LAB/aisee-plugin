...
- context pack 核心契约：[../../plugins/aisee-plugin/references/context-pack-contract.md](../../plugins/aisee-plugin/references/context-pack-contract.md)
...
- ID 规则：[../../plugins/aisee-plugin/references/id-policy.md](../../plugins/aisee-plugin/references/id-policy.md)
- source-map 契约：[../../plugins/aisee-plugin/references/source-map-contract.md](../../plugins/aisee-plugin/references/source-map-contract.md)
- skill 行为：各 `plugins/aisee-plugin/skills/*/SKILL.md`
- schema 事实：`plugins/aisee-plugin/skills/aisee-schema-pack/assets/schema-pack/`
...
- 创建任务、执行任务或建议审查角色前，应先读取已有 workflow state、context pack、schema apply tracks 和可用 skill，优先复用现有 Aisee / Compound 路径。
- App/Web 与硬件/嵌入式必须走不同 domain 适配层，不能共用同一套 UI/服务/数据产物。

## 推荐仓库与插件结构

```text
aisee-plugin/                  # GitHub repo root and marketplace root
  .agents/plugins/marketplace.json
  plugins/aisee-plugin/        # Codex/Claude/Cursor plugin root
    ...
    references/
      ...
      id-policy.md
  src/aisee_cli/               # PyPI CLI package
  scripts/
  README.md
```

Schema pack 的唯一维护源位于 `plugins/aisee-plugin/skills/aisee-schema-pack/assets/schema-pack/`，由 `aisee-schema-pack/scripts/setup-schemas.js` 安装到目标项目的 `openspec/schemas/`。仓库根目录不再保留第二份 `schemas/` 副本。
...
- `aisee bootstrap --plan`：生成项目初始化计划，不直接执行大而全写入。
- `aisee openspec ensure`：窄执行 OpenSpec 初始化，默认运行 `openspec init . --tools none --profile core` 和 `openspec config profile core`。
...
- `aisee bootstrap --plan` 输出迁移建议；当前不提供大而全的 `bootstrap --apply` 写入流程。
...
当 agent 要创建任务、进入实现、提出审查角色或推荐下一步时，必须先复用已有项目工作流：

1. 先用 `aisee:flow` 或 `aisee flow inspect/next` 判断当前 stage。
2. 对单个已确认 change，优先读取 `aisee context pack --change <change> --for <target> --json`。
3. 执行阶段优先走 `aisee:implementation-bridge -> ce-work`；只有 context pack 的 `requires_ce_plan=true`、apply tracks 过粗、路径缺失或 contract 冲突时，才按需使用 `ce-plan`。
4. `ce-plan` 结论不是事实源，必须回写当前 schema apply tracks；只有 source-map schema 才回写 `source-map.md`。
5. Verify/archive 阶段消费 CE review/test/debug evidence，不创建替代 CE 的 code review、test 或 execution agent。

接口、UI、硬件、固件、安全和验证等 domain 差异应作为 schema-aware check lenses 使用，不应扩展成新的全能审查 agent。
