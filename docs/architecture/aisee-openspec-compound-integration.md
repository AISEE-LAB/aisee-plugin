...
- `openspec apply` 不应被替代，它负责把已验证 change 合并进 baseline。
...
Aisee CLI 是 Aisee/OpenSpec/Compound 之间的上下文总线：
...
- `aisee change inspect <change>`：解析单个 OpenSpec change。
- `aisee context pack --change <change> --for <target>`：调用时直接解析 Markdown、OpenSpec artifacts、sources、ID registry 和 source-map，生成给 Aisee skill 或 CE skill 的最小上下文包。
- `aisee id reserve/activate/check`：维护 `.aisee/id-registry.json`。

Aisee CLI 不替代 OpenSpec CLI。它只负责来源登记、模板化解析、查询、初始化编排和 AI 友好 JSON 输出。JSON 是当前 Markdown/artifact 的结构化视图，不是第二份事实源。详细设计见 [aisee-cli-context-and-id-registry.md](aisee-cli-context-and-id-registry.md)。
...
- 通过 apply 把完成且验证通过的 change 合并进 baseline。

OpenSpec `apply` 不是规划工具，也不是实现工具。
...
aisee:change-author
  ├─ proposal.md
  ├─ change-context.md    -> App/Web schema 承接 Architecture 的局部上下文
...
verified
= review/test/verify 通过，等待 apply-guard
...
- 给出是否可以执行 `openspec apply` 的结论。

它不替代 `openspec apply`，只判断 apply 前是否具备条件。

它不应重复 `ce-work` 或 `ce-code-review`。它只读取已有结果，输出短结论：

```text
结论：可以 apply / 暂不建议 apply
阻断项：
- ...
依据：
...
实现后比对代码、tasks、source-map、spec 和验证记录，判断是否可进入 apply。
...
纯软件 change 可能代码测试通过即可 apply；硬件/嵌入式 change 还要考虑样机版本、板级验证、固件版本、生产测试和回归记录。
...
- OpenSpec `apply` 只在 change 已实现、已验证、spec 与代码一致后执行。
...
- 每个 schema 决定自己的 artifact 集合、验证规则和 apply guard 检查表。
...
- 补齐 device/docsite/infra/security 的 context pack 与 apply-guard 检查表。
