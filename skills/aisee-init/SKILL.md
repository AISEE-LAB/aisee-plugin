---
name: aisee-init
description: 初始化、审计并优化 OpenSpec 项目的 Codex 配置体系。用于创建或修复 AGENTS.md、openspec/project.md、.memory/ 和 Codex hooks；检查 hook 机制兼容性、OpenSpec 状态机、规划目录与项目技术上下文边界。触发词包括 aisee:init、初始化项目配置、优化 AGENTS.md、配置 Codex hooks、OpenSpec 配置审计。
---
...
- `docs/requirements/`、`docs/ui-content/`、`docs/tech-context/`、`docs/change-plan/`：aisee 规划链路产物目录。
...
- 是否包含完整状态机：`aisee:srs → aisee:ui-content / aisee:tech-context → aisee:change-plan → /opsx:new --schema aisee-app-spec-driven → 按 schema 生成 artifacts → apply → verify → archive`。
...
- 是否说明 `docs/requirements/`、`docs/ui-content/`、`docs/tech-context/`、`docs/change-plan/` 的职责。
- 若已有规划产物，hook 或配置是否能提示这些目录，但不把它们提升为 OpenSpec artifacts 的替代品。
- 是否明确 `docs/tech-context/` 只提供技术事实和约束，不能替代 `openspec/project.md` 的项目级技术栈来源。
