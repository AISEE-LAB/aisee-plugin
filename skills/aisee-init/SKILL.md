---
name: aisee-init
description: 初始化、审计并优化 OpenSpec 项目的 Codex 配置体系。用于创建或修复 AGENTS.md、openspec/project.md、.memory/ 和 Codex hooks；检查 hook 机制兼容性、OpenSpec 状态机、规划目录与项目技术架构边界。触发词包括 aisee:init、初始化项目配置、优化 AGENTS.md、配置 Codex hooks、OpenSpec 配置审计。
---
...
- `docs/requirements/`、`docs/ui-content/`、`docs/architecture/`、`docs/change-plan/`：aisee 规划链路产物目录。
- `.memory/rules.md` 与 `.memory/index.md`：项目本地记忆规则和记忆入口。
- `docs/reflect/`：会话复盘和 memory 候选区；不替代 `.memory/`。
...
4. 若 `.memory/rules.md` 不存在，用 `assets/memory-rules-template.md` 写入项目本地记忆规则。
5. 若 `.memory/index.md` 不存在，用 `assets/memory-index-template.md` 初始化 `.memory/arch`、`.memory/pref`、`.memory/ctx`、`.memory/stack`。
...
- 记忆规则使用项目本地 `.memory/rules.md`，不再引用或写入任何全局 memory rules 文件。
- `.memory/` 是长期项目记忆的权威位置；`docs/reflect/` 只作为复盘、草案和待确认候选区。
...
- 是否包含完整状态机：`aisee:srs → aisee:ui-content / aisee:architecture → aisee:change-plan → /opsx:new --schema aisee-app-spec-driven → 按 schema 生成 artifacts → apply → verify → archive`。
...
- 是否说明 `docs/requirements/`、`docs/ui-content/`、`docs/architecture/`、`docs/change-plan/` 的职责。
- 若已有规划产物，hook 或配置是否能提示这些目录，但不把它们提升为 OpenSpec artifacts 的替代品。
- 是否明确 `docs/architecture/` 提供技术架构事实、决策和约束，不能替代 `openspec/project.md` 的项目级技术栈来源。
...
- 若项目仍引用全局 memory rules 路径，标记 `[VIOLATION]`，改为项目本地 `.memory/rules.md`。
