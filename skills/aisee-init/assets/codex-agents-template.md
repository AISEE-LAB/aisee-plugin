...
默认推荐 schema 是 `aisee-app-spec-driven`。当 change 使用 `aisee-app-spec-driven` 时，最小闭环 artifacts 为：
...
以下 artifacts 只在 `source-map.md` 的 Artifact 适用性中标记 Required=yes 时展开：
...
如果 Required=no 且写明 N/A 原因，不要为了填模板而展开完整 artifact。
...
- `docs/architecture/`：技术事实、架构决策、项目约束、共享前置和风险来源。
...
- 规划材料不足：先补 `docs/requirements/`、`docs/ui-content/` 或 `docs/architecture/`，不要直接进入实现。
...
- `docs/architecture/`：`aisee:architecture` 输出，说明技术事实、架构决策、项目约束、共享前置和风险；不能替代 `openspec/project.md` 的项目级技术栈事实。
