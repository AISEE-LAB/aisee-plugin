...
description: 安装、审计、创建并维护 OpenSpec schema pack。用于把团队 schema pack 安装到 openspec/schemas/，校验 schema.yaml 和模板路径，检查 artifact requires DAG、apply tracks、config rules 覆盖、schema 命名冲突，并按任务类型推荐 aisee-app-spec-driven、aisee-device-spec-driven、aisee-docsite-driven、quick-fix、quick-research、security-audit、infra-change、opsx-collab-pr-loop 等 schemas。触发词包括 aisee-schema-pack、aisee-opsx-schema、初始化 schema、安装 schemas、检查 OpenSpec schema、schema pack、custom schema、openspec schema validate。
---
...
- 安装本技能自带 schema pack 到目标项目的 `openspec/schemas/`。
- 审计现有 custom schemas 是否符合 OpenSpec 结构和团队最佳实践。
- 根据任务类型推荐合适 schema。
- 创建或调整 schema 时，保持模板路径、依赖图和 apply 语义正确。

`aisee-init` 负责 `CLAUDE.md`、`AGENTS.md`、hooks、memory、`openspec/project.md`。本技能只负责 schemas。
...
node aisee-schema-pack/scripts/setup-schemas.js --schema aisee-docsite-driven --init-baseline
node aisee-schema-pack/scripts/setup-schemas.js --schema quick-fix
node aisee-schema-pack/scripts/setup-schemas.js --schema security-audit --force
```
