...
def _read_api_config_file(path: Optional[str]) -> Dict[str, Any]:
    if not path:
        return {}
    ...
    try:
        ...
        _die(f"Invalid API config JSON: {config_path}: {exc}")
    if not isinstance(data, dict):
        _die(f"API config must be a JSON object: {config_path}")
    return data


def _resolve_api_config(args: argparse.Namespace) -> Dict[str, Optional[str]]:
    config_path = (
        getattr(args, "config", None)
        or os.getenv("AISEE_IMAGEGEN_CONFIG")
        or DEFAULT_CONFIG_PATH
    )
    file_config = _read_api_config_file(config_path)

    api_key = os.getenv("OPENAI_API_KEY") or file_config.get("api_key") or file_config.get("openai_api_key")
    base_url = (
    ...
    if getattr(args, "dry_run", False):
        if not resolved["api_key"]:
            _warn("API key is not configured; dry-run only.")
        if not resolved["base_url"]:
            _warn("Base URL is not configured; dry-run only.")
        return resolved
    ...
    if missing:
        _die(
            ...
            + ", ".join(missing)
            + ". Configure OPENAI_API_KEY and OPENAI_BASE_URL, or provide a JSON config via --config / AISEE_IMAGEGEN_CONFIG."
        )

    print(
        f"API config resolved: base_url={resolved['base_url']}, api_key={_mask_secret(resolved['api_key'])}",
        file=sys.stderr,
    )
    return resolved


def _api_config_preview(args: argparse.Namespace) -> Dict[str, Optional[str]]:
    api_config = getattr(args, "api_config", {}) or {}
    return {
        "base_url": api_config.get("base_url"),
        "api_key": "configured" if api_config.get("api_key") else "missing",
        "config_path": api_config.get("config_path"),
    }
...
def _download_url_bytes(url: str) -> bytes:
    ...
    try:
        ...
        _die(f"Failed to download generated image URL ({exc.code}): {detail}")
    except Exception as exc:
        _die(f"Failed to download generated image URL: {exc}")
    return b""


def _image_item_to_bytes(item: Any) -> bytes:
    if isinstance(item, str):
        return base64.b64decode(item)

    image_b64 = _safe_attr(item, "b64_json")
    if image_b64:
        return base64.b64decode(image_b64)

    url = _safe_attr(item, "url")
    if url:
        return _download_url_bytes(str(url))

    _die("Image response item has neither b64_json nor url.")
    return b""


def _write_image_items_and_downscale(
    items: List[Any],
    outputs: List[Path],
    *,
    force: bool,
...
def _create_client(api_config: Dict[str, Optional[str]]):
    try:
...
def _create_async_client(api_config: Dict[str, Optional[str]]):
    try:
...
async def _run_generate_batch(args: argparse.Namespace) -> int:
            ...
            _print_request(
                {
                    ...
                    "api_config": _api_config_preview(args),
                    "job": i,
    ...
    client = _create_async_client(args.api_config)
    ...
    async def run_job(i: int, job: Dict[str, Any]) -> Tuple[int, Optional[str]]:
        ...
        try:
            ...
            _write_image_items_and_downscale(
                list(result.data),
                outputs,
                force=args.force,
...
def _generate(args: argparse.Namespace) -> None:
    ...
    started = time.time()
    client = _create_client(args.api_config)
...
def _add_shared_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--config",
        help=f"API config JSON path. Defaults to AISEE_IMAGEGEN_CONFIG or {DEFAULT_CONFIG_PATH}.",
    )
    parser.add_argument(
        "--base-url",
        help="OpenAI-compatible API base URL. Overrides OPENAI_BASE_URL and config file base_url.",
    )
...
def main() -> int:
    ...
    if getattr(args, "downscale_max_dim", None) is not None and args.downscale_max_dim < 1:
        _die("--downscale-max-dim must be >= 1")
    args.api_config = _resolve_api_config(args)
