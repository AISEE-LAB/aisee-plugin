...
description: 通用视觉参考图、设计规范与素材生成工作流。用于用户要求基于描述生成 App、微信小程序、H5、PC Web、桌面端或多端参考图；从参考图/截图提取设计规范、StyleSpec、视觉规范；基于既有规范生成新的设计图；规划参考图中的素材需求；生成或编辑背景、插画、装饰层、布局视觉层和生成型透明素材；整理项目本地图片索引、规范索引和素材清单时触发。不要用于 SRS、UI 内容规格、前端实现、Figma 写入、对象分离、去背景、mask 生产或图层包生成。
---

# aisee:design-assets

把用户描述、参考图和已有视觉规范转成项目本地可复用的参考图、StyleSpec、素材和下游视觉 brief。它是轻量工作流 skill，不是完整工作台、需求工具、Figma 写入器或前端实现器。

## 适用输入

- 产品、页面、业务场景或视觉目标描述
- 参考图、截图、草图、品牌图或本地图片路径
- 已有 StyleSpec / 设计规范，Markdown 或 JSON
- 需要生成、编辑或整理的视觉素材、生成型透明素材和素材索引
- 需要按意图查找内置 GPT Image 2 中文提示词素材

判断维度：平台（mobile / mini-program / h5 / pc / desktop / multi）、任务模式（reference / spec / assets / edit / figma-brief / dev-brief / auto）、输出格式（md / json / both）和输出目录。默认输出到目标项目的 `docs/design-assets/`。
...
```bash
find docs/design-assets -maxdepth 3 -type f 2>/dev/null | head -80
...
- 提取或生成 StyleSpec：读 `references/style-spec.md`
...
1. 只有描述且要视觉方向：生成参考图。
2. 有参考图且要沉淀风格：生成 StyleSpec。
3. 有 StyleSpec 且要新页面视觉：基于规范生成新参考图。
...
- `docs/design-assets/index.md`
- `docs/design-assets/index.json`
- `docs/design-assets/manifests/asset-manifest.md`
- `docs/design-assets/briefs/figma-brief.md` / `.json`
- `docs/design-assets/briefs/dev-visual-brief.md` / `.json`

模板见 `assets/`。索引只记录事实、路径、来源和状态，不写冗长过程日志。
