...
- 可以明确写出 `applies_to`，例如 stacks、surfaces、phases、schemas、risk_types。
- 可以写出触发条件，而不是泛泛经验。
...
- <reflect doc, docs/solutions path, review, test, or verified fix>
...
可复用知识候选只是候选卡片，不自动进入全局知识库、用户主目录、其他项目或 active skill。后续需要人工审查、去敏和安装机制。
