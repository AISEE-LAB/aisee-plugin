...
SRS 模块名、页面类型、设计材料、架构层、技术层和 schema artifact 名都只是边界提示，不是 change 名称或最终边界。ID 规则按 `references/source-map-rules.md` 执行：只引用已有正式 ID，不临时发明 ID；`SPEC / API / DATA / TASK / TEST` 属于 change-author 阶段。
