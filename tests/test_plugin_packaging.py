...
def test_plugin_inspect_uses_default_codex_plugin_runtime(tmp_path: Path) -> None:
...
""",
    ...
    assert data["asset_root"].endswith(".codex/.tmp/marketplaces/aisee-plugin/plugins/aisee-plugin")
...
def test_agent_runtime_env_limits_asset_discovery(tmp_path: Path) -> None:
    ...
    data = run_json(tmp_path, "plugin", "inspect", "--json", env={"AISEE_AGENT_RUNTIME": "claude"})

    assert data["asset_root"].startswith("home/.claude/plugins/cache/aisee-plugin/aisee-plugin/9.9.9")


def test_agent_runtime_none_disables_installed_asset_discovery(tmp_path: Path) -> None:
    create_plugin_root(tmp_path / "home" / ".codex" / ".tmp" / "marketplaces" / "aisee-plugin" / "plugins" / "aisee-plugin")

    data = run_json(tmp_path, "plugin", "inspect", "--json", env={"AISEE_AGENT_RUNTIME": "none"})

    assert data["mode"] == "cli-only"
    assert data["asset_root"] is None
    assert data["issues"][0]["code"] == "PLUGIN_CONTENT_UNAVAILABLE"


def test_plugin_inspect_reports_asset_version_for_installed_plugin_content(tmp_path: Path) -> None:
    codex_home = tmp_path / "home" / ".codex"
...
""",
    ...
    assert data["status"] == "risk"
    assert data["asset_version"] == "0.5.0"
