...
把一张输入图处理成可复用对象素材、mask、背景修补图、导出变体和图层包。凡是“基于已有图片提取素材、切出素材、生成透明对象、修补背景或局部重绘”的执行流程，都归本 skill 管理。skill 目录只保存说明、模板和脚本；所有项目产物写入目标项目目录。

## 适用输入

- 本地 PNG/JPG/WebP 图片路径
- 用户要求抠图、去背景、对象分割、透明素材、局部 mask、点选/框选、切图、图层拆分或背景修补
- 用户要求基于图片提取素材、从截图/海报/参考图中切出可复用元素，或把已有图中的主体变成项目素材
...
- 最终可复用素材需要进入项目级视觉资产库时，只把 export/cutout/enhanced 的路径和用途交给 `aisee:design-assets` 更新索引。
...
- 通用素材提取流程：读 `references/asset-extraction.md`
...
2. 有图，要提取一个或多个素材：读 `references/asset-extraction.md`，执行 inventory → region/object selection → mask/cutout → export → index handoff。
...
6. 要人工查看和微调：启动 `select --interactive`，使用 PySide6 双画布 GUI；GUI 支持框选 region、先预览后确认区域属性、从 region 生成 Mask、从 Mask 提取 Cutout、缩略图素材栏、画布定位预览和区域/Cutout 单素材导出。
...
- 多素材提取：先建立候选清单，逐个生成 object/export ID，不把多个素材混成一个对象
...
- 如果是素材提取，列出每个素材的 object/export ID、用途、状态和是否已交给 `aisee:design-assets` 登记
